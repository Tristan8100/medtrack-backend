...
export class AuthService {
  ...
  async signIn(email: string, password: string, role: string): Promise<any> {
    ...
    if (user.role !== role) {
      throw new UnauthorizedException('Invalid Role'); //Same laravel error handling
    }
  ...
  //patient
  async register(register: CreatePatientDTO): Promise<any> {
    const user = await this.usersService.createPatient(register);
    const code = await this.sendVerificationCode(user.email);
    return { message: 'Registration success', email: user.email, code };
  }

  //staff - admin, staff, etc.
  async registerStaff(register: CreateUserDto): Promise<any> {
